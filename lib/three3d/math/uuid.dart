...
class Uuid {
  ...
  static const namespaceNIL = '00000000-0000-0000-0000-000000000000';
  ...
  static List<int> parse(
    ...
    for (Match match in regex.allMatches(uuid.toLowerCase())) {
      if (ii < 16) {
        final hex = uuid.toLowerCase().substring(match.start, match.end);
  ...
  void _initV1() {
    ...
    if (!(_state['hasInitV1']! as bool)) {
      final v1PositionalArgs = (options['v1rngPositionalArgs'] != null)
          ? options['v1rngPositionalArgs']
          : [];
      final v1NamedArgs = (options['v1rngNamedArgs'] != null)
      ...
      final clockSeq = (seedBytes[6] << 8 | seedBytes[7]) & 0x3ffff;
  ...
  void _initV4() {
    ...
    if (!(_state['hasInitV4']! as bool)) {
      ...
      final gPositionalArgs = (options['gPositionalArgs'] != null)
          ? options['gPositionalArgs']
          : const [];
      final gNamedArgs = (options['gNamedArgs'] != null)
  ...
  String v1({Map<String, dynamic>? options}) {
    ...
    _initV1();
    int clockSeq = options['clockSeq'] != null
    ...
    int nSecs = options['nSecs'] != null
        ? (options['nSecs'] as int)
        : (_state['nSecs']! as int) + 1;

    // Time since last uuid creation (in msecs)
    final dt = (mSecs - _state['mSecs']) + (nSecs - _state['nSecs']) / 10000;
    ...
    // time Low
    final tl = ((mSecs & 0xfffffff) * 10000 + nSecs) % 0x100000000;
    buf[i++] = tl >> 24 & 0xff;
    buf[i++] = tl >> 16 & 0xff;
    buf[i++] = tl >> 8 & 0xff;
    buf[i++] = tl & 0xff;
    ...
    // time mid
    final tmh = (mSecs / 0x100000000 * 10000).floor() & 0xfffffff;
    buf[i++] = tmh >> 8 & 0xff;
    buf[i++] = tmh & 0xff;
  ...
  String v4({Map<String, dynamic>? options}) {
    ...
    final namedArgs = (options['namedArgs'] != null)
    ...
    final rnds = options['random'] != null ? options['random'] as List<int> : rng;
  ...
  String v5(String? namespace, String? name, {Map<String, dynamic>? options}) {
    ...
    final useRandom = (options['randomNamespace'] != null)
        ? options['randomNamespace']
        : true;

    // If useRandom is true, generate UUIDv4, else use NIL
    final blankNS = useRandom ? v4() : namespaceNIL;
    ...
    final bytes = parse(namespace);

    // Convert name to a list of bytes
    final nameBytes = <int>[];
    for (final singleChar in name.codeUnits) {
      nameBytes.add(singleChar);
    }

    // Generate SHA1 using namespace concatenated with name
    final hashBytes = crypto.sha1.convert([...bytes, ...nameBytes]).bytes;
  ...
  UuidValue v5obj(String? namespace, String? name,
      {Map<String, dynamic>? options}) {
    final uuid = v5(namespace, name, options: options);
    return UuidValue(uuid);
  }
}
