  ...
  Vector3? getPoint(num t, [Vector? optionalTarget]) {
    ...
    if(optionalTarget is! Vector3){
      if(optionalTarget is Vector4){
        optionalTarget = Vector3().set(optionalTarget.x, optionalTarget.y, optionalTarget.z);
      }
      else{
        optionalTarget = Vector3().set(optionalTarget.x, optionalTarget.y);
      }
    }
    ...
    if (curveType == 'centripetal' || curveType == 'chordal') {
      ...
      num dt0 = Math.pow(p0.distanceToSquared(p1), pow);
      num dt1 = Math.pow(p1.distanceToSquared(p2), pow);
      num dt2 = Math.pow(p2.distanceToSquared(p3), pow);
