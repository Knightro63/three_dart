...
class PMREMGenerator {
  ...
  void _sceneToCubeUV(Scene scene, num near, num far, RenderTarget cubeUVRenderTarget) {
    final fov = 90;
    final aspect = 1;
    ...
    final backgroundMaterial = MeshBasicMaterial({
  ...
  void _halfBlur(RenderTarget targetIn, RenderTarget targetOut, int lodIn, int lodOut, double sigmaRadians, String direction, poleAxis) {
    ...
    final outputSize = _sizeLods[lodOut];
    final x = 3 *
        outputSize *
  ...
  void _setViewport(RenderTarget target, num x, num y, num width, num height) {
