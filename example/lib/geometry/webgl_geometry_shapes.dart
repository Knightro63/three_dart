...
class WebglGeometryShapes extends StatefulWidget {
  WebglGeometryShapes({Key? key, required this.fileName}) : super(key: key);
...
class _MyAppState extends State<WebglGeometryShapes> {
  ...
  Future<void> initPage() async {
    ...
    texture =
        await loader.loadAsync("assets/textures/uv_grid_opengl.jpg");
    loader.dispose();
    ...
    final List<three.Vector2> californiaPts = [];
    ...
    final roundedRectShape = three.Shape();

    void roundedRect(ctx, num x, num y, num width, num height, num radius) {
  ...
  void addShape(shape, extrudeSettings, color, double x, double y, double z, double rx, double ry, double rz, double s) {
    // flat shape with texture
    // note: default UVs generated by three.ShapeGeometry are simply the x- and y-coordinates of the vertices

    three.ShapeGeometry geometry = three.ShapeGeometry([shape]);

    three.Mesh mesh = three.Mesh(
        geometry,
  ...
  void addLineShape(shape, color, double x, double y, double z, double rx, double ry, double rz, double s) {
    ...
    three.Points particles = three.Points(
