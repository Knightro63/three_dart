...
class WebglAnimationMltiple extends StatefulWidget {
  WebglAnimationMltiple({Key? key, required this.fileName})
      : super(key: key);
...
class _State extends State<WebglAnimationMltiple> {
  ...
  List<three.AnimationMixer> mixers = []; // All the three.AnimationMixer objects for all the animations in the scene
  ...
  void initScene2() {
    ...
    //final controls = three_jsm.OrbitControls(camera, _globalKey);

    // ground
    final groundMesh = three.Mesh(three.PlaneGeometry(40, 40),
  ...
  void instantiateUnits() {
    ...
    for (int i = 0; i < UNITS.length; ++i) {
      ...
      if (model != null) {
        final clonedScene = three_jsm.SkeletonUtils.clone(model["scene"]);
   ...
   */
  three.AnimationMixer startAnimation(skinnedMesh, animations, animationName) {
  ...
  void loadGltfModel(model, onLoaded) {
    ...
    loader.load(modelName, (GLTFData gltf) {
  ...
  void animate() {
    ...
    final mixerUpdateDelta = clock.getDelta();
