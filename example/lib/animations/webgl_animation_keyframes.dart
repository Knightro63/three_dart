...
class WebglAnimationKeyframes extends StatefulWidget {
  WebglAnimationKeyframes({Key? key, required this.fileName})
      : super(key: key);
...
class _State extends State<WebglAnimationKeyframes> {
  ...
  void emit(String name, event) {
    final _callbacks = _listeners[name];
    if (_callbacks != null && _callbacks.isNotEmpty) {
      final _len = _callbacks.length;
      for (int i = 0; i < _len; i++) {
        final _cb = _callbacks[i];
        _cb(event);
      }
    }
  }
  ...
  void initPage() async {
    ...
    final pmremGenerator = three.PMREMGenerator(renderer);
    ...
    final loader = three_jsm.GLTFLoader().setPath('assets/models/gltf/test/');

    final result = await loader.loadAsync('tokyo.gltf');
    loader.dispose();
